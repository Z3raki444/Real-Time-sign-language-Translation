...
# ---------- Paths ----------
DATA_ROOT = pathlib.Path("ASL_kaggle")
...
def is_folder_per_class(root: pathlib.Path) -> bool:
    return root.exists() and any(p.is_dir() for p in root.iterdir())

def list_images_recursive(folder: pathlib.Path):
    exts = {".jpg", ".jpeg", ".png", ".bmp"}
    return sorted([p for p in folder.rglob("*") if p.is_file() and p.suffix.lower() in exts])

def make_test_ds_folder_per_class(root, img_size, class_names):
...
def make_test_ds_flat(root: pathlib.Path, labels: list, img_size: int):
...
def plot_confusion_matrix_normalized(cm, labels, out_path):
...
def plot_tp_fp_fn_tn(cm, labels, out_path):
    total = cm.sum()
    tp = np.diag(cm)
    fp = cm.sum(axis=0) - tp
    fn = cm.sum(axis=1) - tp
...
def plot_per_class_accuracy(per_class_acc, labels, out_path):
...
def plot_support(support, labels, out_path):
    x = np.arange(len(labels))
...
def plot_confidence_histogram(y_conf, out_path):
